...
function handleDrop(foodId){if(state!==STATE.PLAYING||isInputLocked)return null;if((foodFeedCount[foodId]||0)>=2)return{result:'exhausted',score:score,combo:combo,correctCount:correctCount,targetCount:targetCount,wrongCount:wrongCount};var correctFoods=currentLevelData.correctFoodIds;var isCorrect=false;for(var i=0;i<correctFoods.length;i++){if(correctFoods[i]===foodId){isCorrect=true;break;}}
if(isCorrect){foodFeedCount[foodId]=(foodFeedCount[foodId]||0)+1;correctCount++;combo++;var bonusPoints=combo>=6?20:combo>=3?15:10;score+=bonusPoints;var encourage=null;if(combo===3)encourage=_randomPick(GAME_CONFIG.ENCOURAGE_COMBO_3);else if(combo===6)encourage=_randomPick(GAME_CONFIG.ENCOURAGE_COMBO_6);return{result:'correct',score:score,combo:combo,correctCount:correctCount,targetCount:targetCount,bonus:bonusPoints,encourage:encourage,animalId:currentLevelData.animalId};}else{wrongCount++;combo=0;return{result:'wrong',score:score,combo:combo,correctCount:correctCount,wrongCount:wrongCount,targetCount:targetCount};}}
